...
export function spotMessage(spot) {
  ...
  return `${spot.ended_at ? '⚫ QRT' : '🟢 CQ SPOT'} · ${spot.callsign}\n${spot.band} m · ${formatFrequency(spot.frequency_hz)} MHz · ${spot.mode}\nLocator ${spot.locator}\n${spot.ended_at ? 'Terminato' : 'In radio'}: ${time} (Roma)`;
}
export async function syncSpot(env, id) {
  ...
  try {
    const result = await telegram(env, spot.message_id ? 'editMessageText' : 'sendMessage', {
      ...
      ...(spot.message_id ? { message_id: spot.message_id } : { message_thread_id: Number(env.TELEGRAM_TOPIC_ID) }),
      text: spotMessage(spot)
    });
